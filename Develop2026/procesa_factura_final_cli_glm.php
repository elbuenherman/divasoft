...
// ============================================================================
//  procesa_factura_final_cli_glm.php
//  Variante CLI usando GLM-4.7 de Z.ai como formateador del fallback OCR
//  (en lugar de Sonnet/Haiku de Anthropic). Misma logica: Haiku doble sobre
//  PDF directo + fallback GLM-OCR -> GLM-4.7 formateador si >20% nulls.
//  Sin HTML, salida texto plano y log dual a archivo.
//
//  Uso: php procesa_factura_final_cli_glm.php <codigo>
//  Ejemplo: php procesa_factura_final_cli_glm.php 79
// ============================================================================
...
$variante           = "GLM-FORMATEADOR";
$modelo_formateador = "glm-4.7";
...
// ----------------------------------------------------------------------------
// ejecuta_ocr_fallback - GLM-OCR -> GLM-4.7 formateador (markdown).
// ----------------------------------------------------------------------------
function ejecuta_ocr_fallback($pdf_binario, $api_zai, $api_anthropic)
    ...
    // --- PASO 2: GLM-4.7 formateador (markdown -> JSON) ---------------------
    ...
    if($err_glm_fmt != "" || $http_glm_fmt != 200)
        return array(
            "ok"               => false,
            "error"            => "GLM-4.7 formateador fallo (HTTP ".$http_glm_fmt.($err_glm_fmt != "" ? " / cURL: ".$err_glm_fmt : "").")",
            "tiempo_glm"       => $tiempo_glm,
    ...
    if($data_glm_fmt === null)
        return array(
            "ok"                => false,
            "error"             => "Respuesta GLM-4.7 formateador no es JSON valido (envoltorio)",
            "tiempo_glm"        => $tiempo_glm,
            "tiempo_glm_fmt"    => $tiempo_glm_fmt,
            "respuesta_glm_fmt" => (string)$resp_glm_fmt,
            "md_results"        => $md_results
    ...
    if(!is_array($extraccion))
        return array(
            "ok"                => false,
            "error"             => "GLM-4.7 formateador devolvio JSON invalido",
            "tiempo_glm"        => $tiempo_glm,
            "tiempo_glm_fmt"    => $tiempo_glm_fmt,
            "respuesta_glm_fmt" => (string)$resp_glm_fmt,
    ...
    // Pricing GLM-4.7: $0.40/M input + $1.75/M output.
    ...
    $costo_g = ($g_in / 1000000.0) * 0.40 + ($g_out / 1000000.0) * 1.75;
...
if($disparar_fallback)
    ...
    log_dual("Disparando GLM-OCR + GLM-4.7 formateador...\n");
