...
$variante           = "HAIKU-FORMATEADOR";
$modelo_formateador = "claude-haiku-4-5-20251001";
...
function ejecuta_ocr_fallback($pdf_binario, $api_zai, $api_anthropic)
    ...
    // --- PASO 2: Haiku formateador (markdown -> JSON) -----------------------
    ...
    $resp_haiku     = curl_exec($ch);
    $http_haiku     = curl_getinfo($ch, CURLINFO_HTTP_CODE);
    $err_haiku      = curl_error($ch);
    $tiempo_haiku   = round(microtime(true) - $t_inicio_haiku, 2);
    curl_close($ch);

    if($err_haiku != "" || $http_haiku != 200)
        return array(
            "ok"             => false,
            "error"          => "Haiku formateador fallo (HTTP ".$http_haiku.($err_haiku != "" ? " / cURL: ".$err_haiku : "").")",
            "tiempo_glm"     => $tiempo_glm,
            "tiempo_haiku"   => $tiempo_haiku,
            "respuesta_glm"  => (string)$resp_glm,
            "respuesta_haiku"=> (string)$resp_haiku,
            "md_results"     => $md_results
            );

    $data_haiku = json_decode((string)$resp_haiku, true);
    if($data_haiku === null)
        return array(
            "ok"              => false,
            "error"           => "Respuesta Haiku formateador no es JSON valido (envoltorio)",
            "tiempo_glm"      => $tiempo_glm,
            "tiempo_haiku"    => $tiempo_haiku,
            "respuesta_haiku" => (string)$resp_haiku,
            "md_results"      => $md_results
    ...
    $texto_haiku = isset($data_haiku["content"][0]["text"]) ? (string)$data_haiku["content"][0]["text"] : "";
    ...
    if(!is_array($extraccion))
        return array(
            "ok"              => false,
            "error"           => "Haiku formateador devolvio JSON invalido",
            "tiempo_glm"      => $tiempo_glm,
            "tiempo_haiku"    => $tiempo_haiku,
            "respuesta_haiku" => (string)$resp_haiku,
            "texto_haiku"     => $texto_haiku,
            "md_results"      => $md_results
            );

    limpia_json_decimales($extraccion);

    $h_in    = isset($data_haiku["usage"]["input_tokens"])  ? (int)$data_haiku["usage"]["input_tokens"]  : 0;
    $h_out   = isset($data_haiku["usage"]["output_tokens"]) ? (int)$data_haiku["usage"]["output_tokens"] : 0;
    $costo_h = ($h_in / 1000000.0) * 1.00 + ($h_out / 1000000.0) * 5.00;
...
if($disparar_fallback)
    ...
    log_dual("Disparando GLM-OCR + Haiku formateador...\n");
    ...
    if(isset($resultado_fallback["respuesta_haiku"]))
        file_put_contents("/tmp/final_".$codigo."_".$fecha_corrida."_fallback_haiku.json", $resultado_fallback["respuesta_haiku"]);
    ...
    if($resultado_fallback["ok"])
        ...
        $tokens_hk  = isset($resultado_fallback["tokens_haiku"]) ? $resultado_fallback["tokens_haiku"] : array();
        ...
        log_dual("\nHaiku formateador llamada:\n");
        log_dual("  Tiempo:        ".$resultado_fallback["tiempo_haiku"]." s\n");
        log_dual("  Tokens in/out: ".number_format((int)(isset($tokens_hk["input"]) ? $tokens_hk["input"] : 0))." / ".number_format((int)(isset($tokens_hk["output"]) ? $tokens_hk["output"] : 0))."\n");
        log_dual("  Costo:         ~$".number_format((float)$resultado_fallback["costo_haiku"], 6)."\n");
...
$tiempo_formateador = ($resultado_fallback !== null && isset($resultado_fallback["tiempo_haiku"])) ? (float)$resultado_fallback["tiempo_haiku"] : 0.0;
...
$costo_formateador = ($resultado_fallback !== null && isset($resultado_fallback["costo_haiku"])) ? (float)$resultado_fallback["costo_haiku"] : 0.0;
