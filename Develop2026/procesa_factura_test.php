...
// ============================================================================
//  procesa_factura_test.php
//  Prueba de extraccion de facturas via Claude API (modelo Haiku 4.5).
//  Uso:  procesa_factura_test.php?codigo=123  (CODIGO del adjunto en archivo_correo)
//  NO inserta en factura_finca ni detalle_factura_finca - solo muestra el JSON.
// ============================================================================
...
// Modelo a usar. Cambiar entre "claude-sonnet-4-6" y "claude-haiku-4-5-20251001".
// Sonnet NO soporta prefill del assistant; Haiku si. La logica se ajusta sola.
...
// Leer el adjunto con sentencia preparada (LONGBLOB completo via store_result).
...
if(!file_exists($ruta_api_key))
    {
    echo "No se encontro el archivo de API key: ".$ruta_api_key;
    exit;
    }
...
// Prompt de extraccion (nowdoc para preservar el texto TAL CUAL).
$prompt = <<<'PROMPT'
...
// Doble extraccion: dos llamadas con distinta temperatura (cinturon-y-tirantes).
...
echo '<title>Prueba doble extraccion - Claude</title>';
echo '<style>';  
...
echo 'pre { background: #f5f5f5; border: 1px solid #ddd; padding: 12px; overflow: auto; max-height: 70vh; font-size: 12px; }';
echo '.meta { font-size: 13px; line-height: 1.7; }';
echo '.meta b { color: #555; }';
echo '.err { color: #88010e; font-weight: bold; }';
echo '.ok { color: #0a7a2d; font-weight: bold; font-size: 16px; }';
echo '.rev { color: #88010e; font-weight: bold; font-size: 16px; }';
echo '</style></head><body>';

echo '<h2>Adjunto</h2>';
echo '<div class="meta">';
echo '<b>CODIGO:</b> '.htmlspecialchars((string)$codigo, ENT_QUOTES, 'UTF-8').'<br>';
...
echo '<b>Tiempo total:</b> '.$tiempo_total.' segundos';
echo '</div>';

// Si alguna llamada fallo, mostrar el error y salir.
if(!$resp1["ok"] || !$resp2["ok"])
    {
    echo '<h2>ERROR en la llamada</h2>';
    if(!$resp1["ok"])
        {
        echo '<h3>Llamada 1 (temp=0)</h3>';
        echo '<p class="err">'.htmlspecialchars((string)$resp1["error"], ENT_QUOTES, 'UTF-8').'</p>';
        if($resp1["respuesta_cruda"] != "")
            echo '<pre>'.htmlspecialchars((string)$resp1["respuesta_cruda"], ENT_QUOTES, 'UTF-8').'</pre>';
        }
    if(!$resp2["ok"])
        {
        echo '<h3>Llamada 2 (temp=0.3)</h3>';
        echo '<p class="err">'.htmlspecialchars((string)$resp2["error"], ENT_QUOTES, 'UTF-8').'</p>';
        if($resp2["respuesta_cruda"] != "")
            echo '<pre>'.htmlspecialchars((string)$resp2["respuesta_cruda"], ENT_QUOTES, 'UTF-8').'</pre>';
        }
    echo '</body></html>';
    exit;
    }

// Procesar ambos textos: prefijar "{", limpiar y parsear.
if($es_sonnet_actual)
    $texto1 = $resp1["texto"];
else
    $texto1 = "{" . $resp1["texto"];
...
// Helpers para usage / costo (precios aproximados Haiku 4.5 USD por 1M tokens).
function render_usage($usage)
    ...
    return "input=".$in." | output=".$out." | cache_creation=".$cc." | cache_read=".$cr;
    } 
 
function calcula_costo($usage)
    {
    // Precios aproximados Haiku 4.5 (USD / 1M tok): input 1.00, output 5.00, cache_read 0.10, cache_write 1.25.
    $in  = isset($usage["input_tokens"]) ? (int)$usage["input_tokens"] : 0;
    $out = isset($usage["output_tokens"]) ? (int)$usage["output_tokens"] : 0;
    $cc  = isset($usage["cache_creation_input_tokens"]) ? (int)$usage["cache_creation_input_tokens"] : 0;
    $cr  = isset($usage["cache_read_input_tokens"]) ? (int)$usage["cache_read_input_tokens"] : 0;
...
$costo1      = calcula_costo($resp1["usage"]);
$costo2      = calcula_costo($resp2["usage"]);
$costo_total = $costo1 + $costo2;
 
echo '<h2>Tokens / Costo</h2>';
echo '<div class="meta">';
echo '<b>Llamada 1 (temp=0):</b> '.htmlspecialchars(render_usage($resp1["usage"]), ENT_QUOTES, 'UTF-8').' &nbsp; ~$'.number_format($costo1, 6).'<br>';
echo '<b>Llamada 2 (temp=0.3):</b> '.htmlspecialchars(render_usage($resp2["usage"]), ENT_QUOTES, 'UTF-8').' &nbsp; ~$'.number_format($costo2, 6).'<br>';
echo '<b>Costo total aproximado:</b> ~$'.number_format($costo_total, 6).' USD (Haiku 4.5: 1.00/5.00/0.10/1.25 por 1M tok)';
echo '</div>';
...
if($estado == 3)
    { 
    echo '<p class="ok">OK (coinciden) &mdash; estado=3</p>';
    }
else
    {
    echo '<p class="rev">REVISAR (discrepan) &mdash; estado=4</p>';
    echo '<h3>Discrepancias ('.count($discrepancias).')</h3>';
    echo '<pre>';
    for($i = 0; $i < count($discrepancias); $i++)
        echo htmlspecialchars($discrepancias[$i], ENT_QUOTES, 'UTF-8')."\n";
    echo '</pre>';
    }

// Mostrar ambos JSON.
echo '<h2>JSON extraccion 1 (temp=0)</h2>';
if(is_array($json1))
    {
    echo '<pre>'.htmlspecialchars(json_encode($json1, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES), ENT_QUOTES, 'UTF-8').'</pre>';
    }
else
    {
    echo '<p class="err">Respuesta 1 no es JSON valido.</p>';
    echo '<pre>'.htmlspecialchars($texto1, ENT_QUOTES, 'UTF-8').'</pre>';
    }

echo '<h2>JSON extraccion 2 (temp=0.3)</h2>';
if(is_array($json2))
    {
    echo '<pre>'.htmlspecialchars(json_encode($json2, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES), ENT_QUOTES, 'UTF-8').'</pre>';
    }
else
    {
    echo '<p class="err">Respuesta 2 no es JSON valido.</p>';
    echo '<pre>'.htmlspecialchars($texto2, ENT_QUOTES, 'UTF-8').'</pre>';
    }
