...
//  PDF directo + fallback GLM-OCR -> Sonnet formateador si >20% nulls.  
...
$variante           = "SONNET-FORMATEADOR";
$modelo_formateador = "claude-sonnet-4-5-20250929";
...
// ----------------------------------------------------------------------------
// ejecuta_ocr_fallback - GLM-OCR -> Sonnet formateador (markdown).
// ----------------------------------------------------------------------------
...
if($disparar_fallback)
    ...
    log_dual("Disparando GLM-OCR + Sonnet formateador...\n");
    ...
    if($resultado_fallback["ok"])
        ...
        log_dual("\nSonnet formateador llamada:\n");
        log_dual("  Tiempo:        ".$resultado_fallback["tiempo_sonnet"]." s\n");
        log_dual("  Tokens in/out: ".number_format((int)(isset($tokens_sn["input"]) ? $tokens_sn["input"] : 0))." / ".number_format((int)(isset($tokens_sn["output"]) ? $tokens_sn["output"] : 0))."\n");
        log_dual("  Costo:         ~$".number_format((float)$resultado_fallback["costo_sonnet"], 6)."\n");
...
$tiempo_formateador = ($resultado_fallback !== null && isset($resultado_fallback["tiempo_sonnet"])) ? (float)$resultado_fallback["tiempo_sonnet"] : 0.0;
...
$costo_formateador = ($resultado_fallback !== null && isset($resultado_fallback["costo_sonnet"])) ? (float)$resultado_fallback["costo_sonnet"] : 0.0;
