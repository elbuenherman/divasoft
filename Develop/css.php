<?php
/// CSS
$url = ""; 
?>
    <meta charset="utf-8">

    <link href="<?php echo $url;?>css/metro-bootstrap.css" rel="stylesheet">
    <link href="<?php echo $url;?>css/metro-bootstrap-responsive.css" rel="stylesheet">
    <link href="<?php echo $url;?>css/iconFont.css" rel="stylesheet">
    <link href="<?php echo $url;?>css/docs.css" rel="stylesheet">
    <link href="<?php echo $url;?>js/prettify/prettify.css" rel="stylesheet">
    <link rel="shortcut icon" href="favicon.ico" type="[COLOR='Red'][B]image/x-icon[/B][/COLOR]">
    <!-- Load JavaScript Libraries -->
    <script src="<?php echo $url;?>js/jquery/jquery.min.js"></script>
    <script src="<?php echo $url;?>js/jquery/jquery.widget.min.js"></script>
    <script src="<?php echo $url;?>js/jquery/jquery.mousewheel.js"></script>
    <script src="<?php echo $url;?>js/jquery/jquery.dataTables.js"></script>
    <script src="<?php echo $url;?>js/prettify/prettify.js"></script>

    <!-- Metro UI CSS JavaScript plugins -->
    <script src="<?php echo $url;?>js/load-metro.js"></script>

    <!-- Local JavaScript -->
    <script src="<?php echo $url;?>js/docs.js"></script>
    <script src="<?php echo $url;?>js/github.info.js"></script>
    
    <!-- Local JavaScript Ajax-->
    <script language="javascript" src="<?php echo $url;?>ajax/ajax.js"></script>
    
<?php
?>